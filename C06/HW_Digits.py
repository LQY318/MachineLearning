import os
import numpy as np
from optimizer_simple import selectJrand, limit_alpha
...
def data_fmt(file_dir):
    ...
    for i in range(len(dir_list)):
        if dir_list[i].split("_")[0] == str(1):  # 取数字１，标签记为１
            ## 读取文件内容
            ...
            label.append(1)
        elif dir_list[i].split("_")[0] == str(9):  # 取数字９，标签记为－１
            ## 读取文件内容
            with open(file_dir + dir_list[i]) as img_file:
                img = img_file.read()
            ## 处理文件内容
    ...
    dataset = np.hstack((feature_m, label_v[:, np.newaxis]))
...
if __name__ == "__main__":
    ## 二分类，只取数字１和９
    ## training
    dataset = data_fmt("./data/trainingDigits/")
    features = dataset[:, :-1]
    labels = dataset[:, -1]
    k1 = 10
    ...
    ## testing
    dataset = data_fmt("./data/testDigits/")
    features = dataset[:, :-1]
    labels = dataset[:, -1]
    error_cnt = 0
    m, n = features.shape
